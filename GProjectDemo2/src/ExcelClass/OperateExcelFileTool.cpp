...
//
CApplication OperateExcelFileTool::excel_application_;


OperateExcelFileTool::OperateExcelFileTool():
...
//初始化EXCEL文件，
BOOL OperateExcelFileTool::InitExcel()
{
	/*
	CoUninitialize();
...
//
void OperateExcelFileTool::ReleaseExcel()
{
...
//打开excel文件
BOOL OperateExcelFileTool::OpenExcelFile(LPCTSTR file_name)
{
...
void OperateExcelFileTool::CloseExcelFile(BOOL if_save)
{
	//如果已经打开，关闭文件
...
void OperateExcelFileTool::SaveasXSLFile(const CString &xls_file)
{
...
int OperateExcelFileTool::GetSheetCount()
{
	return excel_sheets_.get_Count();
}


CString OperateExcelFileTool::GetSheetName(long table_index)
{
	CWorksheet sheet;
...
BOOL OperateExcelFileTool::LoadSheet(long table_index,BOOL pre_load)
{
	LPDISPATCH lpDis = NULL;
	excel_current_range_.ReleaseDispatch();
	excel_work_sheet_.ReleaseDispatch();
...
BOOL OperateExcelFileTool::LoadSheet(LPCTSTR sheet,BOOL pre_load)
{
	LPDISPATCH lpDis = NULL;
	excel_current_range_.ReleaseDispatch();
	excel_work_sheet_.ReleaseDispatch();
...
//得到列的总数
int OperateExcelFileTool::GetColumnCount()
{
	CRange range;
	CRange usedRange;
...
//得到行的总数
int OperateExcelFileTool::GetRowCount()
{
	CRange range;
	CRange usedRange;
	usedRange.AttachDispatch(excel_work_sheet_.get_UsedRange(), true);
...
//检查一个CELL是否是字符串
BOOL OperateExcelFileTool::IsCellString(long irow, long icolumn)
{
	CRange range;
...
//检查一个CELL是否是数值
BOOL OperateExcelFileTool::IsCellInt(long irow, long icolumn)
{
	CRange range;
	range.AttachDispatch(excel_current_range_.get_Item (COleVariant((long)irow),COleVariant((long)icolumn)).pdispVal, true);
	COleVariant vResult =range.get_Value2();
...
BOOL OperateExcelFileTool::IsCellDouble(long irow,long icolumn) {
	CRange range;
	range.AttachDispatch(excel_current_range_.get_Item (COleVariant((long)irow),COleVariant((long)icolumn)).pdispVal, true);
	COleVariant vResult =range.get_Value2();
	//好像一般都是VT_R8
	if(vResult.vt == VT_DECIMAL)       
	{
		return TRUE;
	}
	return FALSE;
...
//
CString OperateExcelFileTool::GetCellString(long irow, long icolumn)
{
...
//
CString OperateExcelFileTool::GetCellStringPrecise(long irow, long icolumn,int precise)
{
	...
	{
		string form	= "%0.";
		form.append(ChangeTypeTool::intToString(precise));
		form.append("f");
		str.Format(form.c_str(),vResult.dblVal);
	}
	//时间格式
	else if(vResult.vt==VT_DATE)    
	{
...
double OperateExcelFileTool::GetCellDouble(long irow, long icolumn)
{
	double rtn_value = 0;
	COleVariant vresult;
	//字符串
...
//VT_R8
int OperateExcelFileTool::GetCellInt(long irow, long icolumn)
{
	int num;
	COleVariant vresult;
...
void OperateExcelFileTool::SetCellString(long irow, long icolumn,CString new_string)
{
...
void OperateExcelFileTool::SetCellInt(long irow, long icolumn,int new_int)
{
...
//
void OperateExcelFileTool::ShowInExcel(BOOL bShow)
{
...
//返回打开的EXCEL文件名称
CString OperateExcelFileTool::GetOpenFileName()
{
	return open_excel_file_;
}

//取得打开sheet的名称
CString OperateExcelFileTool::GetLoadSheetName()
{
	return excel_work_sheet_.get_Name();
}

//取得列的名称，比如27->AA
char *OperateExcelFileTool::GetColumnName(long icolumn)
{   
...
//预先加载
void OperateExcelFileTool::PreLoadSheet()
{
