...
void rayManage:: findFinalCrossPoint(float startPointx,float startPointy,float startPointz,float startDirectiona,float startDirectionb,float startDirectionc,int maxGridx,int maxGridy)//求出最后的交点，得到一些参量
{   
   ...
   double min_distance=5000;//实际中，射线的距离一定比5000小
   ...
   m.rayfinalpointxy(m.get_firstx(),m.get_firsty(),m.get_firstz(),m.get_directiona(),m.get_directionb(),m.get_directionc());//求射线的终点坐标
   findGrid q;//定义一个网格对象
	...
	{       int grid1,grid2;
	   ...
	   for(int t=1;t<=gridNumberTrue;t++)
	   {
		  ...
		  // grid1.GetThisGirdRecord(gridCenterX,gridCenterY);
		   ...
		   //获得一个网格中的建筑物  6.12
		   vector<Building> buildingSet;// = Grid::getGridBuilding(grid1.getGId());
		   for(int i=0;i<buildingSet.size();i++)
		   {
			  // if(grid1.GBId[i]!=0)
				   budnumber++;
		   }
		   ...
		   */
		   ...
		   if (budnumber==0)
			   continue;
		   double a[4],b[4],c[4],d[4],e[4],flaH[4],linepoint1x[4],linepoint1y[4],linepoint2x[4],linepoint2y[4];//a b c代表坐标，d代表是否相交，e代表距离，flaH代表位于建筑物的位置
		   memset(a,0,sizeof(a));
	       memset(b,0,sizeof(b));
		   memset(c,0,sizeof(c));
		   memset(d,0,sizeof(d));
		   ...
		   while(iter!=buildingSet.end()) {
			   int i = 1;
			   Building building1;//定义一个建筑物类，用于查找建筑物的折点
			   building1 = *iter++;
			   int pointnumber;
			   //pointnumber = 0;
			   pointnumber=Building::getBuildingBuildingPoint(building1.getId()).size();
			   float buildingHeight=10; //6.12
			   // h1.getHeightRecord;
			  ...
			  firstReflection1.buildingCross( startPointx,startPointy,startPointz,startDirectiona, startDirectionb, startDirectionc,Building::getBuildingBuildingPoint(building1.getId()),buildingHeight);
		   ...
		   /*
		   for(int i=1;i<=buildingGridSet.size();i++)
		   {
			 ...
			 */  
			   ...
			   */  //暂时作为标注，是求交函数中可以得到的
			  /*FirstReflection firstReflection1;
			  firstReflection1.buildingCross( startPointx,startPointy,startPointz,startDirectiona, startDirectionb, startDirectionc,building1.BPoint,buildingHeight);
			   if(firstReflection1.buildingCrossFlag==0)
				   continue;
			  a[i-1]=firstReflection1.crossX;
			  b[i-1]=firstReflection1.crossY;
		  ...
		  */
		   ...
		   for(int i=1;i<=buildingSet.size();i++)
		   {
			   if(d[i-1]==1)
			   {
		   ...
		   */
		   ...
		   if(f==1&&m.getgroundflag()==1)
		   {
			...
			int rayManage::findPathStop( float pathDistance,float antennaGain )//第一次根据距离判断射线的能量是否降到某一阈值，0表示不停止，1表示停止
			{
				float antennaPower=46;//基站发射的起始功率为46dbm，这里是个参数，是可调的
				float baseStationloss=0;//基站的损耗，这里先考虑为0
				float receivePower;//接收功率
				receivePower=antennaPower-10.9921-20*log10( pathDistance)+antennaGain;//10.9921代表10log4pai
				if(receivePower<-110)//接收端的灵敏度是-109，暂时设为接收端功率的10分之一
					return 1;
				else return 0;
			}
			int rayManage:: findPathRefStop( float pathDistance,float antennaGain,float reflectCofficient)//根据距离和反射系数判断射线的能量是否降到某一阈值，0表示不停止，1表示停止
			{
				float antennaPower=46;//基站发射的起始功率为46dbm，这里是个参数，是可调的
				float baseStationloss=0;//基站的损耗，这里先考虑为0
				float receivePower;//接收功率
				receivePower=antennaPower-10.9921-20*log10( pathDistance)+antennaGain+20*log10(reflectCofficient);//10.9921代表10log4pai
				if(receivePower<-110)//接收端的灵敏度是-109，暂时设为接收端功率的10分之一
					return 1;
				else return 0;
