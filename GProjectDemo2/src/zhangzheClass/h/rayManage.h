...
public:
	void findFinalCrossPoint(float startPointx,float startPointy,float startPointz,float startDirectiona,float startDirectionb,float startDirectionc,int maxGridx,int maxGridy);//求出最后的交点，得到一些参量,输入参数的最后两个指的是地图的水平网格大小和垂直网格大小
	...
	int findPathStop( float pathDistance,float antennaGain ); //第一次根据距离判断射线的能量是否降到某一阈值，0表示不停止，1表示停止
	int findPathRefStop( float pathDistance,float antennaGain,float reflectCofficient); //根据距离和反射系数判断射线的能量是否降到某一阈值，0表示不停止，1表示停止
